...
alias weather2='echo; curl -s wttr.in/{Houston,Hesperia}?format="%20%20%l,%20%c%20%20%C,%20%t,%20%w"; echo'
alias ipinfo='curl -s ipinfo.io | sed "s/[{},\"]//g"; echo'

moon() { echo; curl -s wttr.in/Moon@$1 | head -n 24; }
weather() { echo; curl -s wttr.in/$1 | sed "s/Follow.*//g"; }
weather3() { echo; curl -s wttr.in/{$(echo $@ | sed 's/ /%20/g')}?format="%20%20%l,%20%c%20%20%C,%20%t,%20%w"; echo; }
clbin() { cat $1 | curl -F 'clbin=<-' https://clbin.com; }
traceip() { ping -4 -c 1 $1 | curl -s $(awk 'NR==1 { print "ipinfo.io/"substr($3,2,length($3)-2) }') | sed "s/[{},\"]//g"; echo; }
